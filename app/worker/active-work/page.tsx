...
import { ArrowLeft, ArrowRight, Calendar, Clock, ImageIcon, MapPin, Maximize2, Navigation, Phone, User, X } from 'lucide-react';
...
export default function ActiveWorkPage() {
    ...
    const [workerDetails, setWorkerDetails] = useState<{lat: number, lan: number} | null>(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const workerRes = await axios.get('/api/worker/details');
                console.log(workerRes.data.userDetails)
        if (workerRes.data.success) {
            setWorkerDetails({
                lat: workerRes.data.userDetails.lat,
                lan: workerRes.data.userDetails.lan
            });
        }
        console.log(workerRes.data.userDetails.lat)
        console.log(workerRes.data.userDetails.lan)
        const workRes = await axios.get('/api/all-work');
    ...
    >
        <div className="mb-8">
            ...
            <p className="text-slate-500 text-sm">
                {workerDetails ? "Showing tasks near your registered location" : "Tasks available in your area"}
            </p>
        </div>

        {sortedWorks.length === 0 ? (
            <div className="text-center py-20 border-2 border-dashed border-white/5 rounded-3xl text-slate-500">
            ...
            />
                ...
                >
                    <button 
                        ...
                        className="flex items-center gap-2 text-indigo-400 hover:text-white mb-6 font-bold transition-all"
                    >
                        <ArrowLeft size={20} /> Back to Job Feed
                    </button>
